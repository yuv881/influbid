    ...
    <footer className="bg-brand-dark py-8 border-t border-brand-border/40 relative">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex flex-col md:flex-row items-center justify-between gap-6">
          ...
          {/* Center: Copyright */}
          <div className="text-center md:text-left text-xs text-gray-500 dark:text-gray-400 font-medium tracking-wide">
            © 2026 Influblast • Influencer Marketplace Platform
          </div>

          {/* Right side: Links */}
          <div className="flex items-center gap-6">
            <a href="#privacy" className="text-xs text-gray-500 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white transition-colors font-medium">
              Privacy
            </a>
            <a href="#terms" className="text-xs text-gray-500 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white transition-colors font-medium">
              Terms
            </a>
            <a href="#contact" className="text-xs text-gray-500 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white transition-colors font-medium">
              Contact
            </a>
          </div>
      ...
      {showCookieSettings && (
        <div className="fixed bottom-4 left-4 z-50 max-w-sm w-full bg-brand-card border border-brand-border rounded-xl p-5 shadow-2xl animate-fadeIn backdrop-blur-lg">
          <div className="flex items-start justify-between gap-2 mb-3">
            <h5 className="font-heading text-xs font-bold uppercase tracking-wider text-gray-900 dark:text-white">
          ...
          <div className="flex gap-2">
            <button
              ...
              className="flex-1 bg-brand-orange hover:bg-brand-orangeHover text-gray-900 dark:text-white py-2 rounded-lg font-heading text-[10px] font-black tracking-widest transition-all duration-300"
            >
              ACCEPT ALL
            </button>
            <button
              onClick={() => handleCookieAccept(false)}
              className="flex-1 bg-brand-dark hover:bg-brand-cardHover border border-brand-border text-gray-700 dark:text-gray-300 py-2 rounded-lg font-heading text-[10px] font-black tracking-widest transition-all duration-300"
            >
              OPT OUT
            </button>
          </div>
